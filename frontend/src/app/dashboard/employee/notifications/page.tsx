      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">
            {t("portal.employee.notifications.title")}
          </h1>
        ...
        {!loading && unreadCount > 0 && (
          <Button variant="outline" size="sm" onClick={handleMarkAllRead}>
            {t("portal.employee.notifications.markAllRead")}
          </Button>
        )}
      </div>
      <div className="space-y-3">
        {loading ? (
          ...
          notifications.map((n: any) => (
            <Card
              key={n.id}
              className={`transition-all ${n.status === "READ" ? "opacity-70" : "border-gold/30"}`}
            >
              <CardContent className="p-5">
                <div className="flex gap-4">
                  <div
                    className={`rounded-lg p-2.5 shrink-0 ${n.status === "READ" ? "bg-muted" : "bg-gold/10"}`}
                  >
                    <svg
                      className={`w-5 h-5 ${n.status === "READ" ? "text-muted-foreground" : "text-gold"}`}
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                      strokeWidth={1.5}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <h3
                          className={`text-sm font-medium ${n.status === "READ" ? "text-muted-foreground" : "text-foreground"}`}
                        >
                          {n.title}
                        </h3>
                    ...
                    <div className="flex items-center justify-between mt-3">
                      ...
                      {n.status !== "READ" && (
                        ...
                        >
                          {t("portal.employee.notifications.markRead")}
                        </Button>
                      )}
                    </div>
                  </div>
