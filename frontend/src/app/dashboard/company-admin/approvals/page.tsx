...
import { getPendingCompanyUsers, approveCompanyUser } from "@/lib/api";
export default function CompanyApprovalsPage() {
  ...
  const fetchUsers = () => {
    setLoading(true);
    getPendingCompanyUsers().then((data) => setUsers(Array.isArray(data) ? data : [])).catch(() => setError("Failed to load")).finally(() => setLoading(false));
  };
  ...
  const handleApproval = async (userId: number, approved: boolean) => {
    ...
    setError("");
    try {
      await approveCompanyUser(userId, approved);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground">Pending User Approvals</h1>
        <p className="text-sm text-muted-foreground mt-1">Approve or reject new team member requests</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-sm text-muted-foreground">All team member requests have been processed.</p>
        </div>
      ) : (
