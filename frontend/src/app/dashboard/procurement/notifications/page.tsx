      ...
      <Card>
        <CardContent className="p-0 divide-y divide-card-border">
          {loading ? (
            ...
            <div key={n.id} className={`flex items-start gap-4 p-5 transition-colors ${n.status !== "READ" ? "bg-gold/[0.02]" : ""}`}>
              <div className={`rounded-lg p-2.5 shrink-0 ${n.status !== "READ" ? "bg-gold/10" : "bg-muted"}`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <h3 className={`text-sm ${n.status !== "READ" ? "font-semibold text-foreground" : "font-medium text-muted-foreground"}`}>{n.title || n.message}</h3>
                  {n.status !== "READ" && <span className="w-2 h-2 rounded-full bg-gold shrink-0" />}
                </div>
